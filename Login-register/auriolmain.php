...
$identificador = isset($_GET['id']) ? $_GET['id'] : null;

// Verificar que el identificador no sea nulo y sea un número entero
if ($identificador !== null && is_numeric($identificador)) {
    // Utilizar consulta preparada para evitar SQL injection
    $sql = "SELECT nombre, fecha_nacimiento, rubro, correo, contrasena FROM usuarios WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $identificador);
    $stmt->execute();
    $stmt->bind_result($nombre, $fecha_nacimiento, $rubro, $correo, $contrasena);
            ...
            <script>
                ...
                function enviarFormulario() {
                    if (validarFormulario()) {
                        // Puedes mostrar SweetAlert aquí antes de enviar el formulario si lo deseas
                        Swal.fire({
                            title: '¿Estás seguro?',
